...
const BookDetail = async ({ params,  booksData  }) => {
  const name  = await params
  const book = await booksData.find((item) => item.name == name);
  if (!book) {
    return (
      <div className="container mx-auto px-4 pt-32 text-center">
        <h1 className="text-3xl font-extrabold text-gray-800">Kitab tapılmadı</h1>
  ...
  return (
    <div className="container mx-auto px-4 pt-30 pb-10 max-w-6xl">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 bg-white rounded-3xl shadow-[0_10px_50px_rgba(0,0,0,0.04)] p-6 md:p-12 border border-gray-300">
        ...
        <div className="lg:col-span-5 flex flex-col items-center lg:items-start gap-4">
          <div className="relative w-full aspect-[3/4] max-w-[400px] lg:max-w-none rounded-2xl overflow-hidden shadow-2xl">
            <Image src={book.image} alt={book.title} fill priority sizes="(max-width: 1024px) 100vw, 400px" className="object-cover"
            />
            {book.isBestseller && (
          ...
          <div className="flex items-center gap-2 mt-2 bg-gray-50 px-4 py-2 rounded-xl border border-gray-100 w-fit">
        ...
        <div className="lg:col-span-7 flex flex-col justify-between">
          <div>
            ...
            <div className="mt-6">
              <h3 className="text-sm font-bold uppercase tracking-wider text-gray-400 mb-2">Kitab Haqqında</h3>
              <p className="text-gray-600 leading-relaxed text-base font-light">
                {book.description}
              </p>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-8 bg-gray-50 p-4 rounded-2xl border border-gray-100">
              <div>
